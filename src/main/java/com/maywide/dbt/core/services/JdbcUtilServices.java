...
public class JdbcUtilServices {
    ...
    public void backUpTable(String inDataSource, String targetTable) {
        ...
        if (this.validateTableNameExistByCon(inDataSource, newTable, true)) {
            //删除表
     ...
     * @return
     */
    public boolean existTable(String datasource, String table, boolean isDelete) {
        System.err.println("0000000000000000000000000000");
        return this.validateTableNameExistByCon(datasource, table, isDelete);
    }
    ...
    public void copyTableNoData(String datasource, String oldTable, String newTable) {
        synchronized (JdbcUtilServices.lock) {
            if (!this.existTable(datasource, newTable, true)) {
     ...
     * @return
     */
    public boolean validateTableNameExistByCon(String inDataSource, String tableName, boolean isDelete) {
        DbContextHolder.setDBType(inDataSource);
        ...
        try {
            ...
            dataSource = this.springJdbcTemplate.getDataSource();
            ...
            System.err.println("999999999999");
            log.info(e.getMessage(), e);
            e.printStackTrace();
        } finally {
            try {
        ...
        /*存在的话将表中的数据删除重新添加*/
        if (isExist && isDelete) {
            clearTable(inDataSource, tableName);
        }
        return isExist;
    }
